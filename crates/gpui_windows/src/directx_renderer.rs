...
impl DirectXRenderer {
    ...
    fn render(
        ...
        // Resolve the offscreen scene by blitting it into the swapchain target.
        if use_offscreen {
    ...
    fn dx_blur_pass(
        ...
        unsafe {
            ...
            // Slot 2, not 1 — slot 1 holds BatchParams for the instanced draws (see shaders.hlsl).
            ctx.VSSetConstantBuffers(2, Some(&blur_params));
            ctx.PSSetConstantBuffers(2, Some(&blur_params));
    ...
    fn dx_blur_and_composite(
        ...
        // mask: past it the target is empty anyway, and the sampler wraps, so reading past the
        // target's edge would pull in the opposite side. A blurred group keeps the dilated box —
        // it stays isolated for as long as it exists, and a full-clip pass every frame would
        // cost more than the overflow it recovers.
...
impl DirectXRenderPipelines {
    pub fn new(device: &ID3D11Device) -> Result<Self> {
        ...
        let blur_params_buffer =
            create_constant_buffer_sized(device, std::mem::size_of::<BlurParams>())?;
...
#[inline]
/// Creates a dynamic constant buffer sized at runtime.
///
/// Distinct from the generic [`create_constant_buffer`], which derives its size from a `T` known
/// at compile time — the blur passes size their cbuffer from a plain byte count.
fn create_constant_buffer_sized(device: &ID3D11Device, byte_size: usize) -> Result<ID3D11Buffer> {
